...
$vers  = @('1.0','2.0', '3.0')
$study_subject = "hiddenLayers" # the focus of this experiment 需要更改的参数
$layer_neurons = @(  @(128, 150, 10), @(128, 150, 100, 10),@(128, 256, 150, 100,10) )
$activation_funcs = @( @('None','relu', 'softmax'),  @('None','relu', 'relu', 'softmax'), @('None','relu', 'relu', 'relu', 'softmax'))
$learning_rate = 0.05
...
foreach ($ver in $vers) {
    ...
    foreach ($i in 0..($layer_neurons.Length-1)) {
        $layer_neuron = $layer_neurons[$i]
        $activation_func = $activation_funcs[$i]
        $sub_specifer = $layer_neuron -join '_'
        $run++  

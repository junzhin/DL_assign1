...
$vers  = @('1.0','2.0', '3.0')
$study_subject = "batch_norm" # the focus of this experiment 需要更改的参数
...
# 修改成对应要探索的维度的名字
foreach ($ver in $vers) {
    foreach ($batch_norm in $batch_norms)
    {
        $sub_specifer = $batch_norm # 需要更改的参数
        $run++
        ...
        --weight_decay $weight_decay --beta $beta --size $size --batch_norm $sub_specifer --file_location "../../../raw_data/" --loss $loss --optimizer $optimizer --save_path $save_path > "${save_path}log.txt"

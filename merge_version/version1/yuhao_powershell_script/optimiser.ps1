...
foreach ($ver in $vers) {
    foreach ($i in 0..($optimizer.Length-1)) {
        ...
        --weight_decay $weight_decay --beta $beta --size $size --batch_norm $batch_norm --file_location "../../../raw_data/" --loss $loss --optimizer $sub_specifer --save_path $save_path > "${save_path}log.txt"
